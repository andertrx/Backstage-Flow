...
import { reachRanges, type RunResult, type SyncAccount, syncAccount, syncRange, type SyncStore } from "./runner.ts";
...
function fakeStore() {
  ...
  const store: SyncStore = {
    ...
    startRun: () => (log.push("start"), Promise.resolve(42)),
...
Deno.test("sincroniza na ordem: saldo → estrutura → métricas → alcance, e registra o log", async () => {
  ...
  assertEquals(log, ["start", "token", "saldo", "estrutura", "metricas", "alcance", "fim"]);
