...
async function fetchRows(level: StructureLevel, range: DateRange, args: Record<string, unknown>): Promise<EntityRow[]> {
  ...
  if (error) throw new FriendlyError(friendlyDbError(error, "Não conseguimos carregar os dados."));
...
export function useNames(campaignId: string | null | undefined, adGroupId: string | null | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (c.error || g.error) throw new FriendlyError(friendlyDbError((c.error ?? g.error)!, "Não conseguimos carregar os nomes."));
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .limit(20);
      if (error) throw new FriendlyError(friendlyDbError(error, "Não conseguimos carregar o histórico."));
